      ...
      <Card className="overflow-hidden">
        {isLoading ? (
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {recovering.map((s) => {
                const attempt = s.status === "past_due" ? 1 : 4;
                const nextRetry = s.status === "past_due" ? "Tomorrow" : "Exhausted";
                const accessEnds = s.status === "past_due" ? "In 3 days" : "Immediately";
                ...
                return (
                  <tr key={s.id} className="border-t border-line-2 hover:bg-surface-2">
                    ...
                    <td className="px-4 py-3.5 text-[13px] font-semibold tnum">
                      {formatNaira(s.price.unit_amount)}
                    </td>
                    <td className="px-4 py-3.5">
                    ...
                    <td className="px-4 py-3.5 text-[13px] text-ink-2">{nextRetry}</td>
